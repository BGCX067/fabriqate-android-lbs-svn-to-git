...
public class CloseAppAnimation extends RendererAnimation {
	...
	public void readyFrame(int position) {
		...
		if(position != AnimationConfig.SHOW_ITEM_COUNT / 2)
		{
			...
			mRadius = AnimationConfig.RADIUS + (mMaxStep - mStep) * (AnimationConfig.DEVICE_WIDTH / 2 - AnimationConfig.RADIUS) / mMaxStep;
			
			float height = mMinHeight + (mMaxStep - mStep) * (mMaxHeight - mMinHeight) / mMaxStep;
			float width = mMinWidth + (mMaxStep - mStep) * (mMaxWidth - mMinWidth) / mMaxStep;
